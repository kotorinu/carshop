...
const RE = {
  ...
  btob: /BtoB|B2B|toB向け|法人(の)?(営業|向け|様|お客様|顧客|開拓|担当|企業)|企業(の)?(向け|様|お客様|担当者|開拓)|中小企業|事業者(様|向け)|店舗(オーナー|経営者)(様)?(向け|へ)|代表者様|(企業|法人|事業者|経営者|店舗)[^。\n]{0,6}(向け|様)[^。\n]{0,8}(支援|サポート|コンサル|集客|マーケティング)/i,
  ...
  // 「アポは会社が用意する」と読める言い方を広く拾う。ここが最重要項目なので取りこぼさない
  handover: new RegExp([
    'アポ(イント)?[^。\\n]{0,24}?(お渡し|渡します|渡して|支給|供給|提供|用意|準備|譲渡|固定|手配)',
    '(弊社|当社|会社|こちら)(側)?(で|が|から)[^。\\n]{0,16}?(アポ|商談|お客様|リード|問い合わせ)[^。\\n]{0,10}?(用意|供給|提供|支給|お渡し|集め)',
    '商談(のみ|に専念|に集中|だけ|のみ)',
    '反響営業|インバウンド|待ち(の)?営業',
    '問い合わせ(のあった|いただいた|ベース|をくださった)',
    '広告(運用)?(から|経由|で)[^。\\n]{0,12}?(問い合わせ|反響|リード|集客|集め)',
    '集客は(弊社|当社|会社|こちら)',
    '(新規開拓|飛び込み|テレアポ|架電)[^。\\n]{0,14}?(ありません|不要|なし|しません|一切)',
  ].join('|')),
  ...
  // 「訪問はありません」は stripNegated で消えるので、ここは素の「訪問」も拾ってよい
  onsite: /訪問|対面|来社|常駐|出社|現地|店舗(に|へ)(伺|訪問)|フィールドセールス|ラウンダー|足を運/,
...
/**
 * 打ち消されている表現を消してから判定するための前処理。
 * 「新規開拓や飛び込み営業は一切ありません」を"自己獲得型"と誤判定しないため。
 */
const NEGATABLE = [
  '新規開拓', '飛び込み', 'テレアポ', '架電', '自分でアポ', 'アポ取り',
  '訪問', '対面', '出社', '来社', '常駐', '現地',
  '有形商材', '物販', '商品の販売',
  '初期費用', '登録料', '研修費', '教材費', '参加費', '保証金', '費用のご負担', 'ご負担',
  '学生', '子ども', '子供', 'お子様',
  'ノルマ',
];
const NEGATION = '(ありません|ございません|不要|なし|しません|一切|行いません|お任せしません|かかりません|いただきません|発生しません|必要ありません)';

export function stripNegated(text) {
  const re = new RegExp(`(${NEGATABLE.join('|')})[^。\\n]{0,16}?${NEGATION}`, 'g');
...
export function scoreJob(job, profile = {}) {
  // 危険案件の判定も、打ち消し表現を消してから行う。
  // 「初期費用はかかりません」を危険案件と誤判定して落とすのを防ぐため。
  const text = stripNegated(`${job.title || ''}\n${job.description || ''}\n${job.budget || ''}`);
  const reasons = [];
  const redFlags = [];
 ...
 */
/**
 * 「△（要確認）」を「条件を満たした」とみなしてよい項目。
 *  商材単価 … 推奨レンジ外でも経験にはなる、とFAQにあるのでOK
 * それ以外は、はっきり◯でなければ通さない。
 * 例:「BtoBのお客様も一部います」「アポは支給するが新規開拓もお願いします」は△。
 *    募集文からは"任意"か"必須"かを区別できないので、通さずに面接で確認してもらう。
 */
const WARN_OK = new Set(['price']);
...
export function checkMust(scored, mustKeys = DEFAULT_MUST) {
  ...
  const unconfirmed = mustKeys
    .filter((k) => {
      ...
      return !(c.status === 'warn' && WARN_OK.has(k));
    })
